...
public class ClsAccSheetExport
{
    ...
    public DataTable getAccSheet(int StudentId, int SchoolId,string dateToLoad) //Query to Export the Academic Coversheet
    {
        ...
        try
        {
            ...
            "StdtAcdSheet.DateOfMeeting as StartDate,StdtAcdSheet.ACDateOfMeeting as DateOfMeeting," +
            "StdtAcdSheet.GoalArea,StdtAcdSheet.Goal,ISNULL(stdtacdsheet.benchmarks, '') AS Objective,StdtAcdSheet.MetObjective, StdtAcdSheet.MetGoal,StdtAcdSheet.NotMaintaining, '' as  Curr,StdtAcdSheet.Period1,StdtAcdSheet.Period2,StdtAcdSheet.Period3,StdtAcdSheet.Period4," +
            ...
            "StdtAcdSheet.PersonResNdDeadline,StdtAcdSheet.NoOfTimes1,StdtAcdSheet.NoOfTimes2,StdtAcdSheet.NoOfTimes3,StdtAcdSheet.NoOfTimes4,StdtAcdSheet.NoOfTimes5,StdtAcdSheet.NoOfTimes6,StdtAcdSheet.NoOfTimes7, Mistrial1,Mistrial2,Mistrial3,Mistrial4,Mistrial5,Mistrial6,Mistrial7,StdtAcdSheet.LessonPlanId,ISNULL((SELECT TOP 1 LessonOrder FROM DSTempHdr WHERE DSTempHdr.LessonPlanId=StdtAcdSheet.LessonPlanId AND DSTempHdr.StudentId=" + StudentId + "),0) as LessonOrder " +
            "from StdtAcdSheet inner join Student on StdtAcdSheet.StudentId=Student.StudentId where StdtAcdSheet.StudentId=" + StudentId + "" +
            "and CONVERT(datetime,DateOfMeeting)=CONVERT(datetime,'" + strtDate + "') and CONVERT(datetime,EndDate)=CONVERT(datetime,'" + endDate + "') ORDER BY GoalArea";
            ...
            foreach (DataRow dtRow in Dt.Rows)
            {
                ...
                dtRow["Major"] = string.IsNullOrEmpty(dtNew1) ? "No Results" : dtNew1;
                ...
                dtRow["Minor"] = string.IsNullOrEmpty(dtNew2) ? "No Results" : dtNew2;
                ...
                dtRow["Arrow"] = string.IsNullOrEmpty(dtNew3) ? "No Results" : dtNew3;
