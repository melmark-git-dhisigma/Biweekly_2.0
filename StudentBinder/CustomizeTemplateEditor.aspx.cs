...
public partial class StudentBinder_CustomizeTemplateEditor : System.Web.UI.Page
{
    ...
    static string emailflag = "";
    public string txtUsrids = "";    
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            //ScriptManager.RegisterClientScriptBlock(this, typeof(System.Web.UI.Page), Guid.NewGuid().ToString(), "uncheckEmailSelection();", true);
    ...
    private void setApprovePermission()
    {
        ...
        {
            Approvalcheck();
            //if (sess.perPage.Contains("Email CLP"))
            //{
    ...
    protected void dlLPMailedItemDataBound(object sender, DataListItemEventArgs e)
    {
        //BtnMaintenance.Visible = false;        
        objData = new clsData();
        LinkButton lnkLesson = (LinkButton)e.Item.FindControl("lnkLessonPlanMail");
        if (lnkLesson != null)
            try
            {
                if (lnkLesson.Text.ToString() != null)
    ...
    protected void btnEmailClick(object sender, EventArgs e)             // Submit Template
    {
        ...
        //    TemplateId = Convert.ToInt32(ViewState["HeaderId"]);
        //}
        //LessonPlanId = Convert.ToInt32(objData.FetchValue("select LessonPlanId FROM DstempHdr where DSTempHdrId=" + TemplateId));
        //selQuerry = "SELECT COUNT(LessonName) FROM MailedLP WHERE DSTempHdrId = " + TemplateId + " AND LessonId = " + LessonPlanId + " AND MailStatus = 1";
        //objVal = objData.FetchValue(selQuerry);
        //if (objVal != null)
        //{
    ...
    protected void LoadEmailforSorting()
    {
        ...
        if (sess != null)
        {
            ...
            query = "select UserID as UsrID,UserEmail as Email,UserLName+' '+UserFName+', '+Position as UserName from [User] order by UserLName";


            DataTable dtEmailforSorting = objData.ReturnDataTable(query, false);
            dlEmailforSorting.DataSource = dtEmailforSorting;
            dlEmailforSorting.DataBind();
        }
    }
    protected void LoadEmailforSorting2()
    {
        objData = new clsData();
        sess = (clsSession)Session["UserSession"];
        string query = "";
    ...
    protected void btnEmailSend_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            strQuery = "SELECT Top 1 SMTPPort, SMTPServer,UserEmail,UserPassword FROM EmailDetails";
            DataTable dtNew = objData.ReturnDataTable(strQuery, false);
            if (dtNew != null)
            {
                if (dtNew.Rows.Count > 0)
                {
                    SMTPPort = Convert.ToInt32(dtNew.Rows[0]["SMTPPort"]);
                    SMTPServer = dtNew.Rows[0]["SMTPServer"].ToString();
                    UserEmail = dtNew.Rows[0]["UserEmail"].ToString();
                    UserPassword = dtNew.Rows[0]["UserPassword"].ToString();
                }
            }
            ...
            string selctStudentName = "SELECT StudentLname +', '+ StudentFname As StudentName FROm Student WHERE StudentId = " + sess.StudentId;
            dtNew = objData.ReturnDataTable(selctStudentName, false);
            if (dtNew.Rows.Count > 0)
            {
                StudentName = dtNew.Rows[0]["StudentName"].ToString();
            ...
            if (lstSelecteUserNames.Items.Count > 0)
            {
                string emails = GetCheckedBoxValues();
                string[] emailCollection = emails.Split(new string[] { "@:" }, StringSplitOptions.None);
                string activeMails = emailCollection[0];
                string withoutMails = emailCollection[1];
                string[] individualMails = activeMails.Split(',');
                if (withoutMails != "")
                {

                }
                if (individualMails.Length > 0 && activeMails != "")
                {
                    for (int i = 0; i < individualMails.Length; i++)
                    {
                        string email = individualMails[i];
            ...
            {
                emailStatus = false;
                ScriptManager.RegisterClientScriptBlock(this, typeof(Page), Guid.NewGuid().ToString(), "alertMessage('Please select peoples to send mail','red');", true);
            ...
            if (emailStatus)
            {
                ...
                smtp.Send(message);
                if (objData.IFExists("SELECT MailedId FROM MailedLP WHERE DstempHdrId=" + headerid + " and LessonId=" + LessonPlanId))
                {
                    objData.Execute("Update MailedLP set MailStatus=1,ModifiedBy=" + sess.LoginId + ",ModifiedOn=getdate() WHERE DstempHdrId=" + headerid + " and LessonId=" + LessonPlanId);
                }
                else
                {
    ...
    protected void btnEmailCancel_Click(object sender, EventArgs e)
    {
        objData = new clsData();
        sess = (clsSession)Session["UserSession"];
        ScriptManager.RegisterClientScriptBlock(this, typeof(Page), Guid.NewGuid().ToString(), "$('#').fadeOut();", true);
    }
    protected string GetCheckedBoxValues()
    {
        string emails = "";
        string emailMissing = "";
        foreach (DataListItem li in dlEmailforSorting.Items)
        {

            CheckBox cb = li.FindControl("emailCheckBox") as CheckBox;
            HiddenField email = li.FindControl("hdn_EmailValue") as HiddenField;
            ...
            if (cb != null)
            {
                ...
                if (cb.Checked)
                {
                    //ScriptManager.RegisterClientScriptBlock(this, typeof(Page), Guid.NewGuid().ToString(), "alert("+email.Value+");",true);
                    if (email.Value == null || email.Value.ToString() == "")
                    {
                        emailMissing = emailMissing + cb.Text.ToString()+",";
                    }
                    else
                    {
                        emails = emails + email.Value.ToString()+",";
                    }
                }
        ...
        {
            emailMissing = emailMissing.Remove(emailMissing.Length - 1);
        }
        emails = emails + "@:" + emailMissing;
        return emails;
    ...
    protected void btnSrchEmail_Click(object sender, EventArgs e)
    {
        ...
        if (sess != null)
        {
            string SrchCondtn = txtEmailSrch.Text;            
            if (SrchCondtn != string.Empty)
            {
                dlEmailforSorting.Visible = false;
                dlEmailforSorting_Copy.Visible = true;
                query = "select UserID as UsrID,UserEmail as Email,UserLName+' '+UserFName+', '+Position as UserName from [User] order by UserLName";                

                DataTable dtEmailforSortingCopy = objData.ReturnDataTable(query, false);
                DataView dv = new DataView(dtEmailforSortingCopy);                
                ...
                dlEmailforSorting_Copy.DataBind();

                getSelctedUsrid2();                
                emailflag = hdn_statusFlag.Value;
                if (emailflag == "false")
                {
                    ClientScript.RegisterStartupScript(this.GetType(), "", "loadEmailOrder2('false');", true);
}
                else
                {
                    ClientScript.RegisterStartupScript(this.GetType(), "", "loadEmailOrder2('true');", true);
                }
            }
            else
            {
                emailCheckBox_CheckedChanged(sender, e);
                emailflag = hdn_statusFlag.Value;
                if (emailflag == "false")
                {
                    ClientScript.RegisterStartupScript(this.GetType(), "", "loadEmailOrder2('false');", true);
    ...
    protected string getSelctedUsrid()
    {
        LoadEmailforSorting2();
        foreach (DataListItem li in dlEmailforSorting.Items)
        {
            
            CheckBox cb = li.FindControl("emailCheckBox") as CheckBox;
            HiddenField usrid = li.FindControl("hdn_usrID") as HiddenField;

            if (cb != null)
            {
                if (lstSelectdEmails.Items.Count > 0)
                {
                    foreach (var name in lstSelectdEmails.Items)
                    {
                        if (name.ToString().Equals(usrid.Value.ToString()))
                        {
                            cb.Checked = true;                            
                        }
                    }
                ...
                if (cb.Checked)
                {                    
                    if (usrid.Value != null || usrid.Value.ToString() != "")
                    {
                        txtUsrids = txtUsrids + usrid.Value.ToString() + ",";
                        ...
                        if (!isDuplicate)
                        {
                            lstSelectdEmails.Items.Add(usrid.Value.ToString());
                        }
                    }
                }
                else
                {
                    ...
                    if (isRemoved)
                    {
                        lstSelectdEmails.Items.Remove(usrid.Value.ToString());
                    }
                }
            }
        ...
        if (txtUsrids != null && txtUsrids.Length > 1)
        {
            txtUsrids = txtUsrids.Remove(txtUsrids.Length - 1);
        }
        return txtUsrids;
    }

    protected string getSelctedUsrid2()
    {
        foreach (DataListItem li in dlEmailforSorting_Copy.Items)
        {

            CheckBox cb = li.FindControl("emailCheckBox_Copy") as CheckBox;
            HiddenField usrid = li.FindControl("hdn_usrID_Copy") as HiddenField;
    ...
    protected void emailCheckBox_CheckedChanged(object sender, EventArgs e)
    {
        dlEmailforSorting.Visible = true;
        dlEmailforSorting_Copy.Visible = false;
    ...
    protected void emailCheckBox_CheckedChanged2(object sender, EventArgs e)
    {
        foreach (DataListItem li in dlEmailforSorting.Items)
        {
            CheckBox cb = li.FindControl("emailCheckBox") as CheckBox;
    ...
    protected void updateUsernames()
    {
        string[] qryUsrNameAry = new string[0];

        if (lstSelectdEmails.Items != null)
        {
            string Usrids = "";
            qryUsrNameAry = new string[lstSelectdEmails.Items.Count];
            for (int i = 0; i < lstSelectdEmails.Items.Count; i++)
            {
                qryUsrNameAry[i] = lstSelectdEmails.Items[i].ToString();
            }
            Usrids = String.Join(",", qryUsrNameAry);
            if (Usrids != "")
            {
                string qryUsrName = "SELECT UserID as UsrID,UserLName+' '+UserFName+', '+Position as UserName from [User] WHERE UserId IN(" + Usrids + ")order by UserLName";
                DataTable dtUsr = objData.ReturnDataTable(qryUsrName, false);
                if (dtUsr != null)
                {
                    lstSelecteUserNames.DataSource = dtUsr;
                    lstSelecteUserNames.DataTextField = "UserName";
                    lstSelecteUserNames.DataValueField = "UsrID";
                    lstSelecteUserNames.DataBind();
                }
            }
        }
    }

    protected void btnEmailSelction_Click(object sender, EventArgs e)
    {
        if (dlEmailforSorting.Visible == true)
        {
            emailCheckBox_CheckedChanged2(sender, e);
        }
        else if (dlEmailforSorting_Copy.Visible == true)
        {
            emailCheckBox_CheckedChanged(sender, e);
        }
    }

    protected void btnDeltMails_Click(object sender, EventArgs e)
    {
        objData = new clsData();
        if (lstSelecteUserNames.Items.Count > 0 && lstSelectdEmails.Items.Count > 0)
        {
            if (lstSelecteUserNames.SelectedIndex > -1)
            {
                string value = lstSelecteUserNames.SelectedItem.Value;
                string text = lstSelecteUserNames.SelectedItem.Text;
                ListItem item = new ListItem(); //create a list item
                item.Text = text;               //Assign the values to list item   
                item.Value = value;
                lstSelecteUserNames.Items.Remove(item); //Remove from the selected list

                ListItem item1 = new ListItem(); //create a list item
                item.Text = value;               //Assign the values to list item   
                item.Value = value;
                lstSelectdEmails.Items.Remove(item); //Remove from the selected list
    ...
    protected void emailReset(object sender, EventArgs e)
    {
        dlEmailforSorting.Visible = true;
        dlEmailforSorting_Copy.Visible = false;
    ...
    protected void emailReset2(object sender, EventArgs e)
    {
        foreach (var name in lstSelectdEmails.Items)
        {
            foreach (DataListItem li in dlEmailforSorting_Copy.Items)
    ...
    protected void emailFullReset(object sender, EventArgs e)
    {
        if (dlEmailforSorting.Visible == true)
        {
            txtEmailSrch.Text = "";
            lstSelecteUserNames.Items.Clear();
            lstSelectdEmails.Items.Clear();
            foreach (DataListItem li in dlEmailforSorting.Items)
            {
                CheckBox cb = li.FindControl("emailCheckBox") as CheckBox;
                HiddenField usrid = li.FindControl("hdn_usrID") as HiddenField; 
    ...
    {
        string strSql = " SELECT RGP.ReadInd,RGP.WriteInd FROM RoleGroupPerm RGP  INNER JOIN UserRoleGroup URG ON RGP.RoleGroupId = URG.RoleGroupId INNER JOIN  [Object] O ";
        strSql += "ON RGP.ObjectId = O.ObjectId WHERE    URG.SchoolId =  " + sess.SchoolId + " AND  URG.UserId = " + sess.LoginId + "  and O.ObjectName='Lesson Email-Button'";
    ...
    {
        string strSql = " SELECT RGP.ReadInd,RGP.WriteInd FROM RoleGroupPerm RGP  INNER JOIN UserRoleGroup URG ON RGP.RoleGroupId = URG.RoleGroupId INNER JOIN  [Object] O ";
        strSql += "ON RGP.ObjectId = O.ObjectId WHERE    URG.SchoolId =  " + sess.SchoolId + " AND  URG.UserId = " + sess.LoginId + "  and O.ObjectName='Lesson Submit-Button'";
    ...
    protected void Approvalcheck()
    {
        string strSql = " SELECT RGP.ReadInd,RGP.WriteInd FROM RoleGroupPerm RGP  INNER JOIN UserRoleGroup URG ON RGP.RoleGroupId = URG.RoleGroupId INNER JOIN  [Object] O ";
        strSql += "ON RGP.ObjectId = O.ObjectId WHERE    URG.SchoolId =  " + sess.SchoolId + " AND  URG.UserId = " + sess.LoginId + "  and O.ObjectName='Lesson Approve-Button'";
