        ...
        try
        {
            ...
            lock (_lock)
            {
                using (StreamWriter sw = new StreamWriter(fullPath, true))
                {
                    ...
                    {
                        sw.WriteLine(
                            "ReportName,StudentId,ClassId,UserId,ServerID,StartTime,EndTime,DurationMs,RowCount,Status,Parameters,ErrorMessage");
                    }

                    sw.WriteLine(
                        Escape(log.ReportName) + "," +
                        Escape(log.StudentId.ToString()) + "," +
                        Escape(log.ClassId.ToString()) + "," +
