...
public partial class Home : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            sesserrlog.WriteToLog(DateTime.Now.ToString() + ',' + sess.LoginTime.ToString() + ',' + sess.LoginId.ToString() + ',' + preid.ToString() + ',' + sess.UserName + ',' + preuser + ',' + sess.SchoolId + ',' + sess.SessionID + ',' + curesesid + ',' + "Student Binder:Home" + ',' + sess.Classid + ',' + sess.StudentId + ',' + HostName + ',' + userAgent);

            Pagepath = "Home:Page  load";
    ...
    public static string SetStudentID(string stdID)
    {
        ...
        try
        {
            if (_studID != "")
            {
                ...
                string Sessstname = HttpContext.Current.Session["Sprestname"].ToString();
                ClsSessionErrorlog sesserrlog = new ClsSessionErrorlog();
                sesserrlog.WriteToLog(DateTime.Now.ToString() + ',' + oSession.LoginTime.ToString() + ',' + oSession.LoginId.ToString() + ',' + preid.ToString() + ',' + oSession.UserName + ',' + preuser + ',' + oSession.SchoolId + ',' + oSession.SessionID +',' + curesesid + ','+ "Student Binder:Home set student" + ',' + oSession.Classid + ',' + oSession.StudentId + ',' + HostName + ',' + userAgent);
            ...
            string Pagepath = "Home : Set student";
    ...
    public static clsSubmenu[] FillSubMenu(string Menu)
    {
        ...
        string Sessstname = HttpContext.Current.Session["Sprestname"].ToString();
        string curesesid = HttpContext.Current.Session.SessionID.ToString();
        string Pagepath = "Home : Fill submenu";
    ...
    public static void setTemplateSess(string StudIDs)
    {
        ...
        if (StudID != 0)
        {
            ...
            try
            {
                ...
                string Pagepath = "Home:Set TemplateSess";
                sess = clsGeneral.sessioncheck(curesesid, preid, preuser, sess, Prevsess, SessStudentid, Sessstname, Pagepath);
...
" )) " + optString + ") DATA WHERE LessonOrder >" + lastitem + " ORDER BY  DATA.LessonOrder";
 ...
 " LookupName='Maintenance')) " + optString + ") DATA WHERE LessonOrder >" + lastitem + " ORDER BY  DATA.LessonOrder";
    ...
    protected void imgSearchDsch_Click(object sender, ImageClickEventArgs e)
    {
        ...
        {
            studentDetail = "SELECT distinct ST.StudentPersonalId AS Id, ST.FirstName+' ' +ST.LastName AS Name FROM StudentPersonal ST  join StdtClass stcl on ST.StudentPersonalId=stcl.StdtId where stcl.ClassId=(select ClassId from Class where ClassCd='dsch')";
        }
        else
        {
            studentDetail = "SELECT Id, Name FROM (SELECT distinct STP.StudentPersonalId AS Id, STP.FirstName+' ' +STP.LastName AS Name FROM StudentPersonal STP  join StdtClass stcl on STP.StudentPersonalId=stcl.StdtId where stcl.ClassId=(select ClassId from Class where ClassCd='dsch') AND (STP.FirstName='" + clsGeneral.convertQuotes(txtSnameDsch.Text.Trim()) + "' " +
